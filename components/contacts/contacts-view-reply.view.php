...
<div id="bordo"> <!--inizio #bordo-->
     	...
     	<h2 class="titolo">Gestione Risposte e Inoltro</h2>
...
<?php echo Forms::button("avvia ricerca","submit",'pulsante-invia')?>
	...
	<?php echo Forms::hidden("t",$task)?>
            ...
            <div id="inserisci-elemento">
            	...
            	<a href="<?php echo Utils::makeLink($module,"newReply")?>" title="">inserisci indirizzo</a>
...
<td class="tleft"><?php echo $row->title?></td>
		...
		<td class="small">
		<?php echo Utils::href(Utils::makeLink($module,"validateReply",array("id"=>$row->id,"validate"=>$validate)),$validate_text,array("class"=>$validate_class));?>
		</td>
<td class="small">
		<?php echo Utils::href(Utils::makeLink($module,"openReply",array("id"=>$row->id)),'modifica',array("class"=>"button"));?>
		</td>
		<td class="small ultimo">
		<?php 
			echo Utils::href(Utils::makeLink($module,"deleteReply",array("id"=>$row->id)),"cancella",array("onclick"=>'return confirm(\'Sei sicuro di voler eliminare questo elemento?\')',"class"=>"button red"));
		?>
		</td>
</tr>
<?php endforeach;?>

  ...
  class analytics{
    ...
    */
    private function auth(){
        ...
        if ($_SESSION['auth'] == ''){
            unset($_SESSION['auth']);
            Log::error('Retrieving Auth hash failed!');
        }
        $this->_sAuth = $_SESSION['auth']; 
    }
    ...
    */
    private function getUrl($sUrl, $aPost = array(), $aHeader = array()){
        ...
        if (function_exists('curl_init')){
            ...
            if ($sOutput === false){
                Log::error('Curl error (' . curl_error($rRequest) . ')');    
            }
            ...
            if ($aInfo['http_code'] != 200){
                ...
                if ($aInfo['http_code'] == 400){
                    Log::error('Bad request (' . $aInfo['http_code'] . ') url: ' . $sUrl);     
                }
                if ($aInfo['http_code'] == 403){
                    Log::error('Access denied (' . $aInfo['http_code'] . ') url: ' . $sUrl);     
                }
                Log::error('Not a valid response (' . $aInfo['http_code'] . ') url: ' . $sUrl);
            }
            ...
            if (strpos($http_response_header[0], '200') === false){
                // not a valid response from GA   
                Log::error('Not a valid response (' . $http_response_header[0] . ') url: ' . $sUrl);       
            }
        }
        return $sOutput;
    }   
    ...
    */    
    public function getBrowsers(){
        ...
        $aData = $this->getData(array(  'dimensions' => 'ga:browser',
                                        'metrics'    => 'ga:visits',
                                        'sort'       => 'ga:visits'));             
        arsort($aData);
        return $aData;                                                                                                                                                                           
